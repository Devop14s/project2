...
Assert-ContainsAll -FilePath (Join-Path $DocsRoot 'jenkins-bootstrap-runbook.md') -Tokens @(
    '`dockerhub-creds`',
    '`kubeconfig-file`',
    '`SOURCE_REPO_URL`',
    '`SOURCE_REPO_REF`',
...
Assert-ContainsAll -FilePath (Join-Path $DocsRoot 'failure-triage.md') -Tokens @(
    '`source bootstrap`',
    '`registry/auth`',
    '`agent tooling`',
    '`cluster access`',
