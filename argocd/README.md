...
- The local `validate-argocd-apps` helpers now verify that both manifests keep the expected repo URL, `main` target revision, Helm path, environment values file, and `CreateNamespace=true` sync option.
