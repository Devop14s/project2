...
The regenerated GitOps overlays now also carry the environment-specific ingress hosts for both `storefront` and `backoffice`, so ArgoCD does not fall back to the static defaults from `helm/yas/values.yaml`.
